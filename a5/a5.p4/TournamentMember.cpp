...
void Player::GoalSorter(Player &a,Player &b,Player &c)
{
    while(c>b || b>a || c>a)
    {
        if(c>b)
...
Referee::Referee(char *a,char *b,char *c,double d,string e,string f) :
    TournamentMember(a,b,c,d,e,f)
{
    yellowCardCount=0;
    redCardCount=0;
...
bool Referee::addToYellowCardList(Player *p)
{
    for(int i=0; i<yellowCardCount; i++)
    {
        ...
        {
            cout<<"The player already has a yellow card"<<endl;
...
bool Referee::addToRedCardList(Player *p)
{
    for(int i=0; i<redCardCount; i++)
    {
        if(redCardList[i]==p)
        {
            cout<<"The player already has a red card"<<endl;
            return false;
        }
    }
    redCardList[redCardCount]=p;
    redCardCount++;
    cout<<"Red Card added"<<endl;
    return true;
}
